...
const supabaseRoutes = new Elysia({
  ...
  .post('/create', async ({ supabaseService, body, set }) => {
    const { data, error } = await supabaseService.create((body as any).table, (body as any).data);
    if (error) { set.status = 400; return { success: false, message: error.message }; }
  ...
  .put('/update', async ({ supabaseService, body, set }) => {
    const { data, error } = await supabaseService.update(
      (body as any).table, (body as any).id, (body as any).data,
    );
    if (error) { set.status = 400; return { success: false, message: error.message }; }
    return { success: true, data };
  })
  ...
  .delete('/delete', async ({ supabaseService, body, set }) => {
    const { data, error } = await supabaseService.delete((body as any).table, (body as any).id);
    if (error) { set.status = 400; return { success: false, message: error.message }; }
    return { success: true, data };
  });
