...
const scraperRoutes = new Elysia({
  ...
  .post('/scrape', async ({ body }) => {
    ...
    const safe = typeof result === 'object' && result !== null
      ? { ...result, content: (result as any).content?.replace(/\r\n?/g, '\n') ?? '' }
      : result;
  ...
  .get('/financialjuice', () => ({ success: true, data: scrapedContentStore.get('financialjuice') }))
  .get('/yahoofinance',   () => ({ success: true, data: scrapedContentStore.get('yahoofinance')   }))
  .get('/coinmarketcap',  () => ({ success: true, data: scrapedContentStore.get('coinmarketcap')  }))
  ...
  .get('/sources', async () => {
    ...
    return { success: true, data: await AppDataSource.getRepository(ScrapingSourceEntity).find() };
  })
