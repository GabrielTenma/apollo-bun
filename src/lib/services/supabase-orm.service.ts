...
export class SupabaseOrmService {
	...
	async findOne<T>(entity: any, id: number | string): Promise<T | null> {
		return this.getRepository(entity).findOneBy({
			id,
		} as any) as unknown as T | null;
	}
	...
	async delete<_T>(entity: any, id: number | string): Promise<void> {
