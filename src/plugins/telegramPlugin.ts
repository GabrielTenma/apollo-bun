...
export interface TelegramPluginContext {
  ...
  sendMessage: (opts: { chat_id: number | string; text: string; parse_mode?: string; reply_markup?: unknown }) => Promise<unknown>;
  /** Shortcut — chat_id + text + optional parse mode. */
  sendText:    (chatId: number | string, text: string, parseMode?: 'Markdown' | 'MarkdownV2' | 'HTML') => Promise<unknown>;
...
export const telegramPlugin = new Elysia<TelegramPluginContext>({ name: 'Telegram' })
